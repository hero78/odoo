...
class PaymentTransaction(models.Model):
    ...
    def action_cinetpay_set_done(self):
        """ Set the state of the cinetpay transaction to 'done'.
        ...
        self._handle_notification_data('cinetpay', notification_data)

    def action_cinetpay_set_canceled(self):
    ...
    def action_cinetpay_set_error(self):
    ...
    def _send_refund_request(self, **kwargs):
        ...
        refund_tx._handle_notification_data('cinetpay', notification_data)
    ...
    def _get_tx_from_notification_data(self, provider_code, notification_data):
        ...
        if provider_code != 'cinetpay' or len(tx) == 1:
            return tx

        reference = notification_data.get('reference')
        tx = self.search([('reference', '=', reference), ('provider_code', '=', 'cinetpay')])
        if not tx:
            raise ValidationError(
                "Cinetpay: " + _("No transaction found matching reference %s.", reference)
            )
        return tx
    ...
    def _process_notification_data(self, notification_data):
        ...
        self.provider_reference = f'cinetpay-{self.reference}'
        ...
        if self.tokenize:
            ...
            self._cinetpay_tokenize_from_notification_data(notification_data)
        ...
        if state == 'pending':
            ...
            self._set_error(_("You selected the following cinetpay payment status: %s", state))

    def _cinetpay_tokenize_from_notification_data(self, notification_data):
        ...
        token = self.env['payment.token'].create({
            ...
            'verified': True,
            'cinetpay_simulated_state': state,
        })
        self.write({
            'token_id': token,
            'tokenize': False,
